...
- **攻击面最小**：没有多模态解析，就没有内存溢出，也没有复杂的二进制处理。
- **财务最稳**：因为不做多模态，"多扣费"策略逻辑是自洽的，没人能拿一张图来攻击计费系统。
- **开发最快**：只需要处理 JSON 文本流，不需要处理 multipart/form-data 或 base64 图片解码。

一句话：放弃了 30% 的炫酷功能，换来了 100% 的落地确定性。这是架构师的胜利。
